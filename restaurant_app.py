...
class UI:
    ...
    def __init__(self, root: ThemedTk):
        ...
        s = ttk.Style()
        s.theme_settings("equilux", {
            "TButton": {
                "configure": {"padding": 0},
                "map": {
                    "foreground": [("active", "green2"), ("!disabled", "green4"), ("focus", "#4cd137")],
                    "background": [("active", "green2"), ("!disabled", "green4"), ("focus", "#4cd137")],
                }
            },
        })

        s.configure('.', background=self.bg_color, font="{U.S. 101} 15 bold", foreground="white")
        ...
        ### COMMON LABELS
        title = ttk.Label(common_frame, text="Restaurant Management", style='title.TLabel')
        ...
        for index, item in enumerate(article_list):
            ttk.Label(article_frame, text=item).place(x=195, y=20+index*30, anchor="e")
        ...
        ### PRICE
        prices = ["Discount %", "Cost", "Service Charge", "Tax", "SubTotal", "Discount", "Total"]
        for index, price_txt in enumerate(prices):
            ...
            if price_txt == "Discount %":
                self.add_buttons(common_frame, price_entry, 0.55, 0.25 + 0.07*index, 100)
        ...
        for i in range(9):
            ...
            temp_button = ttk.Button(common_frame, text=i+1, command=lambda x=i+1: self.add_to_entry(self.res_entry, x))
            temp_button.place(relx=0.75 + 0.05*column, rely=0.28 + 0.11*line, width=button_width, height=button_height)

        for index, operand in enumerate(self.operands):
            operand_button = ttk.Button(common_frame, text=operand, command=lambda x=operand: self.add_to_entry(self.res_entry, x))
            operand_button.place(relx=0.9, rely=0.28 + 0.11*index, width=button_width, height=button_height)
        zero_button = ttk.Button(common_frame, text=0, command=lambda: self.add_to_entry(self.res_entry, 0))
        zero_button.place(relx=0.75, rely=0.61, width=button_width, height=button_height)
        reset_button = ttk.Button(common_frame, text='C', command=self.erase_calc)
        reset_button.place(relx=0.80, rely=0.61, width=button_width, height=button_height)
        dot_button = ttk.Button(common_frame, text=".", command=lambda: self.add_to_entry(self.res_entry, '.'))
        dot_button.place(relx=0.85, rely=0.61, width=button_width, height=button_height)
        equal_button = ttk.Button(common_frame, text="=", command=lambda: self.compute_string(self.res_entry, self.res_entry.get()))
        equal_button.place(relx=0.75, rely=0.72, width=button_width*4 +4, height=button_height)

        ### CONTROL BUTTONS
        index = 0
        button_labels = {"TOTAL": self.calc_total,
        ...
        for key, value in articles.items():
            ...
            value_label = ttk.Label(price_frame, text=value, style='yfood.TLabel', justify="center")
            value_label.place(relx=0.7, rely=0.1 + 0.15*index)
            index += 1

    @property
    def bill(self):
        return '\n'.join([f"{label} : {entry.get()}" for label, entry in self.price_entries.items()])
    ...
    def display_total(self, price: float):
        ...
        self.price_entries["Service Charge"].insert(0, f"{service:.02f} {self.currency}")
    ...
    def add_buttons(self, frame: ttk.Frame, entry: ttk.Entry, entry_x: float, entry_y: float, width: int):
        """Add button to add or remove food from the bill"""
        add_button = ttk.Button(frame, text='+', command=lambda x=entry: self.increment(x), style='small.TButton')
        add_button.place(relx=entry_x+width/1000-0.002, rely=entry_y, width=20, height=20)
        minus_button = ttk.Button(frame, text='-', command=lambda x=entry: self.decrement(x), style='small.TButton')
